import {list} from '../services/safetyEventLog';
import {DEFAULT_PAGE_SIZE} from '../constants/config';

export default {
  namespace: 'safetyEventLog',
  ...
  effects: {
    *list({payload = {}}, {call, put, select}) {
      ...
      const prevPagination = yield select(
        state => state.safetyEventLog.pagination,
      );
      const prevFilter = yield select(state => state.safetyEventLog.filter);
      const prevSorter = yield select(state => state.safetyEventLog.sorter);
      ...
      const {data, pagination} = yield call(list, {
        ...nextFilter,
