import {list, remove, create} from '../services/attentionDeploymap';
import {DEFAULT_PAGE_SIZE} from '../constants/config';
export default {
  namespace: 'attentionDeploymap',
  ...
  effects: {
    *list({payload = {}}, {call, put, select}) {
      ...
      const prevPagination = yield select(
        state => state.attentionDeploymap.pagination,
      );
      const prevFilter = yield select(state => state.attentionDeploymap.filter);
      const prevSorter = yield select(state => state.attentionDeploymap.sorter);
