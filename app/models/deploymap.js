...
export default {
  ...
  effects: {
    *list({payload = {}}, {call, put, select}) {
      ...
      // 获取分页列表数据的情况
      const prevPagination = yield select(state => state.deploymap.pagination);
      const prevFilter = yield select(state => state.deploymap.filter);
      const prevSorter = yield select(state => state.deploymap.sorter);
